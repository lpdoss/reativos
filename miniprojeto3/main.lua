...
function mqtt_cb(topic, message)
  ...
  elseif string.sub(message, 1, string.len("Falha")) == "Falha" then
    mensagem = "Falha no pedido de geolocalização."
  else 
    mensagem = message
  end
...
function love.load()
  ...
  APIKey_Canal_textbox = gui_objects:newTextbox(10, 70, "alterar_chave")
...
function love.update(dt)
  ...
  if PedidoLocal_flag == 1 then
    ...
    if PedidoLocal_timerFlag % 180 == 0 then
      PedidoLocal_flag = 2
      PedidoLocal_timerFlag = 0
    end
  end
...
function love.draw()
  ...
  love.graphics.printf("Canal para envio do pedido:", 10, (love.graphics.getHeight()/5)*2 + 50, 200, "left", 0, 1, 1)
  PedidoLocal_CanalEnvio_textbox.draw()
  love.graphics.printf("Canal para monitoramento da reposta:", 220, (love.graphics.getHeight()/5)*2 + 50, 200, "left", 0, 1, 1)
  PedidoLocal_CanalResposta_textbox.draw()
  PedidoLocal_Pedir_button.draw()
  love.graphics.print("Timeout: 180 segundos.", 550, (love.graphics.getHeight()/5)*2 + 90, 0, 1, 1)
  love.graphics.print("Aperte 'F2' para abortar.", 550, (love.graphics.getHeight()/5)*2 + 110, 0, 1, 1)
  
  -- Resposta
  love.graphics.print("Última resposta recebida:", 10, (love.graphics.getHeight()/5)*3 + 15, 0, 2, 2)
  love.graphics.print("-----------------------------------------------------------------------------", 10, (love.graphics.getHeight()/5)*3 + 45)
  if PedidoLocal_flag == 1 then
    local timer = math.floor(PedidoLocal_timerFlag)
    love.graphics.print("Aguardando resposta do pedido de localização (" .. timer .. " seg)..." , 10, (love.graphics.getHeight()/5)*3 + 65, 0, 1.5, 1.5)
  elseif PedidoLocal_flag == 2 then
    love.graphics.print("O tempo do pedido de localização expirou, tente novamente." , 10, (love.graphics.getHeight()/5)*3 + 65, 0, 1.5, 1.5)
  elseif PedidoLocal_flag == 3 then
    love.graphics.print("Pedido de localização abortado." , 10, (love.graphics.getHeight()/5)*3 + 65, 0, 1.5, 1.5)
  else
    love.graphics.print("Canal: ", 10, (love.graphics.getHeight()/5)*3 + 65, 0, 1.5, 1.5)
    love.graphics.printf(topico, 75, (love.graphics.getHeight()/5)*3 + 68, love.graphics.getWidth() - 10, "left", 0, 1.25, 1.25)
    love.graphics.print("Mensagem: ", 10, (love.graphics.getHeight()/5)*3 + 95, 0, 1.5, 1.5)
    if type(mensagem) == "table" then
      local posY = (love.graphics.getHeight()/5)*3 + 98
      ...
      -- valor inesperado
      love.graphics.printf(mensagem, 120, (love.graphics.getHeight()/5)*3 + 98, love.graphics.getWidth() - 10, "left", 0 , 1.25, 1.25)
    end
  end
end
